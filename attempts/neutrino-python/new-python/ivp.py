    ...
    for t_ini in [p.t_ini]:
        for _ in range(1):
            ...
            sol = solve_ivp(func, (t_ini, p.t_fin), y0,
                            method=p.metodo, args = (H0re, H0im, D),
            ...
            plt.plot(t / p.solar, sobrev_array, label=r'$P(\nu_e \to \nu_e)$')
            plt.plot(t / p.solar, mu_array, label=r'$P(\nu_e \to \nu_\mu)$')
            plt.plot(t / p.solar, tau_array, label=r'$P(\nu_e \to \nu_\tau)$')
            #plt.plot(t / p.solar, 1-sobrev_array, label=r'$P(\nu_e \to \nu_\mu)$')
            plt.xlabel(r'$r = R/R_\odot$', fontsize=20)
            plt.ylabel(r'Probabilidade', fontsize=20)
            plt.ylim(0, 1.05)
            plt.legend(fontsize=14)
            plt.title(p.titulo)
            plt.savefig(str(p.num_nu)+"nu-"+p.metodo+".png", dpi=300, format='png', bbox_inches="tight")
            #'''
