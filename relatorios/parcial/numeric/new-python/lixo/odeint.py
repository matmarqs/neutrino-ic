...
#   p_prod = p_prod / sum(p_prod)   # normalizando p_prod
    ...
    for t_ini in [0]:
        for _ in range(1):
            ...
            # EDO
            t   = np.linspace(t_ini, p.t_fin, p.num_it)
            y0  = np.array([p.re1, p.re2, p.re3, p.im1, p.im2, p.im3])
            y   = odeint(func, y0, t,   # odeint
                         atol=p.eps_abs_odeint, rtol=p.eps_rel_odeint,
                         args = (H0re, H0im, D))
            ...
            plt.title(p.titulo)
            plt.savefig("2nu.png", dpi=300, format='png', bbox_inches="tight")
